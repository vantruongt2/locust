...
class CategoryNavigate(SequentialTaskSet):
    ...
    def navigate_to_women_category(self):
        with self.client.get("/id=3&controller=category", headers=self.header,
    ...
    def navigate_to_dresses_category(self):
        with self.client.get("/id=5&controller=category", headers=self.header,
                             catch_response=True) as response:
            if response.status_code != 200:
    ...
    def navigate_to_shirt_category(self):
        with self.client.get("/id=8&controller=category", headers=self.header,
                             catch_response=True) as response:
            if response.status_code != 200:
